...
import trrakee.pnavw.util.AngleLowpassFilter;
...
public class RadarScanView extends View implements SensorEventListener {
    ...
    public RadarScanView(Context context, AttributeSet attrs, int defStyle) {
        ...
        mGridPaint.setColor(getResources().getColor(R.color.purdue_lighter));
        ...
        mSweepPaint0.setColor(ContextCompat.getColor(context, R.color.purdue));
        ...
        mSweepPaint1.setColor(ContextCompat.getColor(context, R.color.purdue));
        ...
        mSweepPaint2.setColor(ContextCompat.getColor(context, R.color.purdue));
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            mBlip = ((BitmapDrawable) Objects.requireNonNull(ContextCompat.getDrawable(context, R.drawable.ic_location_on_black_24dp))).getBitmap();
        }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.drawCircle(center, center, (radius * 3) >> 2, gridPaint);
    ...
    private void updateBeaconsInfo(final Collection<Beacon> beacons) {
        mInfoView.setText(format(getResources().getString(R.string.text_scanner_found_beacons_size), beacons.size()));
    }
    ...
    private void updateDistanceText(double distanceM) {
        ...
        if (mUseMetric) {
            String mMetricDisplayFormat = "%.0fm";
            displayDistance = format(mMetricDisplayFormat, distanceM);
        } else {
            String mEnglishDisplayFormat = "%.0fft";
            float FEET_PER_METER = 3.28084f;
            displayDistance = format(mEnglishDisplayFormat, distanceM * FEET_PER_METER);
        }
